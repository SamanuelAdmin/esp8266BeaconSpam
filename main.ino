...
const bool wpa2 = true; // if it is WPA2 networks
...
const char ssids[] PROGMEM = {
  ...
  "The Creep Next Door\n"
  "Ye Olde Internet\n"
  "IGIL used to this network\n"
  "Your mom loves me\n"
  "Yes\n"
  "No\n"
  "I gonna kill your dog\n"
};
...
Adafruit_SSD1306 display(0); //3x11
...
void nextChannel() { // Shift out channels one by one (changing channels)
...
void randomMac() { // Random MAC generator
  for (int i = 0; i < 6; i++){
...
void setup() {
  for (int i = 0; i < 32; i++) emptySSID[i] = ' ';  // create empty SSID

  randomSeed(os_random()); // for random generator

  
  packetSize = sizeof(beaconPacket); // set packetSize
  if (wpa2) {
  ...
  WiFi.mode(WIFI_OFF); // start WiFi
  wifi_set_opmode(STATION_MODE);

  wifi_set_channel(channels[0]); // Set to default WiFi channel

  Serial.println("SSIDs:"); // Display all saved WiFi SSIDs
  int i = 0;
...
void loop() {
  ...
  if (currentTime - attackTime > 100) { // send out SSIDs
  ...
  if (currentTime - packetRateTime > 1000) {
    ...
    display.setTextColor(WHITE);
    display.setCursor(32, 8);
    display.println("Packets/s: ");
    display.setCursor(32, 24);
    display.println("  " + (String) packetCounter);
    display.display();
